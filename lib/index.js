...
function EmblaCarousel(sliderRoot) {
  ...
  function scrollBy(progress) {
    var distance = engine.scrollBy.progress(progress);
    engine.scrollBody.useDefaultMass().useDefaultSpeed();
...
exports["default"] = EmblaCarousel; //export { UserOptions }
// @ts-ignore
...
function Engine(root, container, slides, options, events) {
  ...
  var viewportSize = utils_1.rectWidth(root);
  ...
  var scrollLimit = scrollLimit_1.ScrollLimit({
    loop: loop,
    contentSize: contentSize,
    isContained: containerSize <= viewportSize
  });
...
function ScrollBy(params) {
  ...
  function withinBounds(distance) {
    var desiredTarget = target.get() + distance;
...
function ScrollLimit(params) {
  ...
  function measure(scrollSnaps) {
    ...
    var max = startSnap; // if slides inside viewport min =0;
...
function ScrollTarget(params) {
  ...
  function findTargetSnap(target) {
    ...
    var ascDiffsToSnaps = scrollSnaps.map(function (scrollSnap) {
      ...
      return shortestWay(diffToSnap, 0);
    }).map(function (diff, i) {
      return {
        diff: diff,
        index: i
  ...
  function shortestWay(target, direction) {
    var t1 = target;
  ...
  function byIndex(index, direction) {
    ...
    var distance = shortestWay(diffToSnap, direction);
    return {
      index: index,
      distance: distance
    };
  ...
  function byDistance(distance, snap) {
    ...
    var snapDistance = distance + shortestWay(diffToSnap, 0);
    return {
      index: index,
...
function Translate(node) {
  var roundToTwoDecimals = utils_1.roundToDecimals(2);
  var translate = 0; // function translateX(n: number): string {
  //   return `translate3d(${n}%,0px,0px)`
  // }

  function translateXPx(n) {
    var w = Math.round(utils_1.rectWidth(node) / 100 * n);
    return "translate3d(".concat(w, "px,0px,0px)");
  }
